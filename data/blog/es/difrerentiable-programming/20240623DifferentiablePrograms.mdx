...
$$
\begin{equation}
\text{max-pooling}(x) = \max_{j \in [\,d\,]} x_j.
\end{equation}
$$
...
$$
\begin{equation}
\text{log-sum-exp}(x) = \log\sum_{j=1}^{d} \exp(x_j).
\end{equation}
$$
...
$$
\begin{equation}
\operatorname*{softplus}((x, 0)) = \operatorname*{softplus}(x).
\end{equation}
$$
...
$$
\begin{equation}
\text{log-sum-exp}(x) = \text{logsumexp}(x - c\mathbf{1}) + c,
\end{equation}
$$
