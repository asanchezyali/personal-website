...
vector de parámetros $\theta_k \in \Theta_k,$ donde $\Theta_k$ es el espacio de parámetros de la función $f_k.$ Por lo
tanto,
...
\begin{aligned}
s_0 &\in S_{0}, \\
s_{1} &= f_{1}(s_{0}; \theta_1) \in S_{1}, \\
s_{2} &= f_{2}(s_{1}; \theta_2) \in S_{2}, \\
&\vdots \\
s_{n} &= f_{n}(s_{n-1}; \theta_n) = f(s_0; \theta) \in S_{n}.\\
\end{aligned}
\end{equation}
$$

para una entrada $s_0\in S_0$ y para los **parámetros entrenables** $\theta = (\theta_1, \ldots, \theta_n) \in \Theta =
\Theta_1 \times \cdots \times \Theta_n.$ Cada función $f_k$ es una **capa** de la red y cada $s_k \in S_k$ es
...
función $f:S_0 \times \Theta \to S_n$ que mapea una entrada $s_0$ y parámetros $\theta$ a una salida $s_n.$

Dado un programa parametrizado de este tipo, los parametros $\theta$ podemos aprender los parámetros ajustando $\theta$
de acuerdo a un conjunto de datos de entrenamiento. Por ejemplo, dado un conjunto de datos de pares $(x_i, y_i),$ podemos
minimizar la función de pérdida $L(\theta) = \sum_{i=1}^{m} \ell(f(x_i; \theta), y_i).$ La minimización de la función de
