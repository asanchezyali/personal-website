...
date: '2022/10/05'
tags: ['Python', 'operator-overloading', 'numpy', 'numpy ufuncs', 'modular arithmetic', 'modular arithmetic', 'lights out game']
draft: false
time: '18 min'
summary: 'In this tutorial, we're going to learn how to create a library for modular arithmetic, using operator overloading and redefining the built-in functions for NumPy.'
bibliography: references-data.bib
canonicalUrl: 'https://www.asanchezyali.com/blog/en/modular-arithmetic.html'
headerImage: 'INSERT IMAGE HERE'
language: 'en'
---
...
This matrix is not the inverse in modular arithmetic with modulo 
, mainly because the result is in decimals and not integers. Another way we could try is:
...
Mathematically, we will name the hours of the clock as INSERT EQUATION HERE and list it like this:

INSERT EQUATION HERE

The elements of this set are all the possible remainders left by the integers when divided by 12. Note that instead of listing 12 (mod 12), 0 (mod 12) has been listed because INSERT EQUATION HERE.
...
For the Lights Out Game, each bulb will be modeled as a light that switches between a finite list of colors in a cyclic sequence. Therefore, we can use modular arithmetic INSERT EQUATION HERE to tell the computer what state each of the bulbs is in.

As we saw in the clock example (in general in INSERT EQUATION HERE), it’s possible to add hours together if we take into account the cycle of the hours. For example, if we want to calculate (13 + 15) (mod 12), we’ll add 13 and 15 together to get 28. Then, we find that the number 12 divides into 28 a total of 2 times with 4 left over. We can write this as:
...
Similarly, we can also do subtraction and multiplication. In general, addition, subtraction and multiplication are defined over INSERT EQUATION HERE by the following formulae:

INSERT EQUATION HERE

Moreover, if n is prime, in the set INSERT EQUATION HERE, we can calculate the divisions using the [Euclidean algorithm](https://en.wikipedia.org/wiki/Euclidean_algorithm).
...
### Step 1: Build a Python class to manipulate the elements of INSERT EQUATION HERE
...
Note that Zmodn receives only the variable module, which allows defining the set of equivalence classes INSERT EQUATION HERE according to the value of the module. To generate the elements of INSERT EQUATION HERE, we redefine the __call__ method, which allows us to call our class as a function to build different instances of Zmodn by congruence_class = self.__class__(self.module), and calculate the representative (self.representative) of the class as the remainder when dividing integer by self.module. Finally, we redefine the method __repr__ to be represented by the console Zmodn class as a (mod n).

To see how this works, let’s generate the elements of INSERT EQUATION HERE. To do this, we first write:
...
As we saw in my post about [operator overloading](https://monadical.com/posts/operator-overloading-in-python.html), it’s possible to redefine all the basic Python operations +, -, X, /.

The next step is to redefine the methods __add__(+), __sub__(-), __mul__(X), and __truediv__ (/). For this, we’ll make use of what we learned in the operator overloading post. So we have:
...
Note that before calculating any operation, we must ensure that the elements that we’re adding are calculated with respect to the same module. For example, it doesn’t make sense to add an element of INSERT EQUATION HERE with an element of INSERT EQUATION HERE. We also verify that other is an instance of Zmodn by using isinstance(other, self.__class__). Then we compute the operations (+, -, x, /) with respect to the class representatives to finally return the resulting class.

As for the division between two elements a and b of INSERT EQUATION HERE, it only makes sense if b is [coprime](https://en.wikipedia.org/wiki/Coprime_integers) with n. In that case, we first make use of the [Euclidean algorithm](https://en.wikipedia.org/wiki/Euclidean_algorithm) for remainders and then calculate the multiplicative inverse of b. So we have:
...
And therefore, we can use it to do our calculations on any set of type INSERT EQUATION HERE. Let’s look at some examples:
...
### Create arrays using Zmodn and NumPy functionalities
...
To complete this class, we add the __add__, __sub__ and __mul__ methods. This has a similar structure to the methods already implemented in Zmodn, the only difference here is that the operations (+,-,X) are executed through NumPy, and we make use of the astype(int) method to obtain the integer representatives of the Zmodn classes. These methods would be:
...
### Calculate the inverse matrix with modular arithmetic using ZmodnArray
...
So far, we’ve seen how to build a Zmodn class that allows us to work with the INSERT EQUATION HERE modular classes, and we learned how to use these elements with NumPy.
