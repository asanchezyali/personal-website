...
export default function ArticleOverview({ title, summary, date, headerImage, slug, tags, time }) {
  ...
  return (
    <div className="group h-full overflow-hidden rounded-lg border border-gray-200 bg-white shadow-md transition-all duration-300 hover:shadow-lg dark:border-gray-700 dark:bg-gray-800">
      ...
      <div className="flex flex-grow flex-col justify-between p-5">
        <div>
          <Link href={`/blog/${slug}`} className="block">
            <h2 className="mb-2 line-clamp-2 text-xl font-bold text-gray-900 hover:text-primary-500 dark:text-white dark:hover:text-primary-400">
              {title}
            </h2>
          </Link>
