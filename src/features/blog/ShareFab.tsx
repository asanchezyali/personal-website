...
export default function ShareFab({ title, slug }: ShareFabProps) {
  const [copied, setCopied] = useState(false)

  const getUrl = () => (typeof window !== 'undefined' ? window.location.href : `https://asanchezyali.com/${slug}`)
  ...
  function shareX() {
    window.open(`https://x.com/intent/tweet?url=${encodeURIComponent(getUrl())}&text=${encodeURIComponent(title)}`, '_blank')
  }
  ...
  return (
    <div className="share-fab">
      ...
      <button onClick={shareFacebook} aria-label="Share on Facebook" title="Share on Facebook">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
