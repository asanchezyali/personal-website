...
export default function ShareFab({ title, slug, locale }: ShareFabProps) {
  const { t } = useTranslation(locale, 'common')
  const [copied, setCopied] = useState(false)

  const getUrl = () =>
    typeof window !== 'undefined' ? window.location.href : `https://asanchezyali.com/${slug}`
  ...
  return (
    <div className="share-fab">
      <span className="share-fab-label">{t('share')}</span>
      ...
      <button onClick={shareX} aria-label={`${t('shareon')} X`} title={`${t('shareon')} X`}>
