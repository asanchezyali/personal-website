...
export default function TransformGallery({ labels = {} }: TransformGalleryProps) {
  ...
  const mapped = SHAPE.map(([x, y]) => [A[0] * x + A[1] * y, A[2] * x + A[3] * y] as [number, number])
    ...
    <div className="tg">
      <div className="tg-canvas">
        <svg viewBox={`0 0 ${SIZE} ${SIZE}`} role="img" aria-label={names[kind]}>
          <clipPath id="tg-clip"><rect x={0} y={0} width={SIZE} height={SIZE} /></clipPath>
      ...
      <div className="tg-controls">
        <div className="chooser-opts">
          {(Object.keys(names) as Kind[]).map((k) => (
            <button key={k} type="button"
                    ...
                    onClick={() => { setKind(k); setT(RANGES[k].init) }}
                    aria-pressed={k === kind}>
              {names[k]}
            </button>
          ))}
        ...
        <label className="mplay-slider">
          ...
          <input type="range" {...RANGES[kind]} value={t}
                 onChange={(e) => setT(Number(e.target.value))} aria-label="parameter" />
          <span className="mplay-val">{kind === 'scale' || kind === 'shear' ? t.toFixed(1) : `${t}°`}</span>
        </label>

        <div className="mmul-bracket tg-matrix">
          <div className="mmul-cells" style={{ gridTemplateColumns: 'repeat(2, minmax(46px, auto))' }}>
            {A.map((x, i) => <span key={i} className="mmul-cell">{f(x)}</span>)}
          </div>
        </div>
