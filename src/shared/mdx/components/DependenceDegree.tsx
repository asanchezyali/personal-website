...
function rSquared(X: number[][], y: number[]) {
  ...
  for (let i = 0; i < cols; i++) {
    G.push(Array.from({ length: cols }, (_, j) => X.reduce((s, row) => s + row[i] * row[j], 0)))
...
export default function DependenceDegree({ labels = {} }: DependenceDegreeProps) {
  ...
  const { sigmas, cond, vif, rank } = useMemo(() => {
    ...
    const G = [0, 1, 2].map((i) => [0, 1, 2].map((j) => X.reduce((s, r) => s + r[i] * r[j], 0)))
    const ev = eigSym3(G)
  ...
  return (
    <div className="ddeg">
      ...
      <div className="ddeg-controls">
        ...
        <p className="cs-note">
          {labels.note ??
            'The rank flips at the very end; the condition number and the VIF grow throughout.'}
        </p>
