...
const CLOUD: [number, number][] = [
  [-2.4, -1.4], [-1.8, -0.6], [-1.2, -1.0], [-0.6, -0.1],
  [0.0, 0.3], [0.6, 0.2], [1.2, 0.9], [1.8, 0.7],
  [2.4, 1.5], [-0.3, -0.7], [0.9, 0.6], [-1.5, -1.2],
]
  ...
  return (
    <div className="bsub">
      <div className="bsub-canvas">
        <svg viewBox={`0 0 ${SIZE} ${SIZE}`} role="img" aria-label={labels.hint ?? 'Best line through a cloud'}>
          <clipPath id="bsub-clip"><rect x={0} y={0} width={SIZE} height={SIZE} /></clipPath>
          ...
          <g clipPath="url(#bsub-clip)">
            <line className={atBest ? 'bsub-line is-best' : 'bsub-line'}
                  x1={toX(-far * u[0])} y1={toY(-far * u[1])}
                  x2={toX(far * u[0])} y2={toY(far * u[1])} />
            {centred.map(([x, y], i) => {
              const t = x * u[0] + y * u[1]
              return (
                <line key={`r${i}`} className="bsub-res"
                      x1={toX(x)} y1={toY(y)} x2={toX(t * u[0])} y2={toY(t * u[1])} />
              )
            })}
            {centred.map(([x, y], i) => (
      ...
      <div className="bsub-controls">
        <p className="dvec-hint">{labels.hint ?? 'The line turns; the segments are the distances being squared and summed.'}</p>
        ...
        <svg className="bsub-curve" viewBox="0 0 100 36" preserveAspectRatio="none" aria-hidden="true">
          <polyline points={curve} />
          <line className="bsub-mark" x1={((deg + 90) / 180) * 100} y1={0}
                x2={((deg + 90) / 180) * 100} y2={36} />
        </svg>
        <p className="bsub-axis" aria-hidden="true"><span>−90°</span><span>0°</span><span>90°</span></p>

        <p className="mplay-readout" aria-live="polite">
          {labels.residual ?? 'residuo'} = {residual.toFixed(2)} ·{' '}
          {labels.variance ?? 'explicado'} {((explained / total) * 100).toFixed(1)} %
        </p>
