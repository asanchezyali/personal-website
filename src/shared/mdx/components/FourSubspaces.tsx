    ...
    <div className="fs">
      <div className="fs-stage">
        <svg
          viewBox={`0 0 ${W} ${H}`}
          role="img"
          aria-label={labels.hint ?? 'The four fundamental subspaces'}
        >
          ...
          <text
            className="fs-lab"
            x={lx + boxW / 2}
            y={pad + leftTop + (usable - leftTop) / 2 + 4}
            textAnchor="middle"
          >
            ker · {nullity}
          </text>
          ...
          <text
            className="fs-lab"
            x={rx + boxW / 2}
            y={pad + rightTop + (usable - rightTop) / 2 + 4}
            textAnchor="middle"
          >
            kerᵀ · {coker}
          </text>
          ...
          {rank > 0 && (
            <line
              className="fs-arrow is-bij"
              x1={lx + boxW + 4}
              y1={pad + leftTop / 2}
              x2={rx - 8}
              y2={pad + rightTop / 2}
              markerEnd="url(#fs-head)"
            />
          )}
          {/* the kernel collapses to the origin: a single point of the codomain */}
          {nullity > 0 && (
            <line
              className="fs-arrow is-zero"
              x1={lx + boxW + 4}
              y1={pad + leftTop + (usable - leftTop) / 2}
              x2={rx - 14}
              y2={pad + rightTop - 1}
            />
          )}
          <circle className="fs-origin" cx={rx - 8} cy={pad + rightTop - 1} r={4} />
          <text className="fs-cap" x={rx - 8} y={pad + rightTop + 15} textAnchor="middle">
            0
          </text>
      ...
      <div className="fs-controls">
        <p className="dvec-hint">
          {labels.hint ?? 'The blocks are sized by the dimension they hold.'}
        </p>
        ...
        <ul className="fs-legend">
          <li>
            <i className="is-bij" /> {labels.bijection ?? 'row space → image, one to one'}
          </li>
          <li>
            <i className="is-zero" /> {labels.toZero ?? 'kernel → 0'}
          </li>
          <li>
            <i className="is-coker" /> {labels.unreachable ?? 'left null space: never reached'}
          </li>
        </ul>
